...
- `targeting_automation: { advantage_audience: 0 }` — **forçado 0** (rígido, sem sugestão da Meta). O espelhamento do público (decisão 2026-07-10, PR #77) foi **revertido em 2026-07-27**: `advantage_audience=1` + `PROFILE_VISIT` disparava o erro Meta **#2016153** ("not eligible for Profile Visit ads") no review, numa conta que roda FASE 1 normal — antes do #77 (adv=0 fixo) não dava esse erro. Reaver o espelhamento (idealmente como toggle na UI p/ contas elegíveis) = `git revert` do commit do fix. Ver `buildFase1Targeting` em `meta-publish`.
